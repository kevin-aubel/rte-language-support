...
function provideCompletionItems(_document, _position) {
  ...
  const builtins = [
    ...
    ['rename',            'rename(${1:source}, ${2:destination})',                    'Renommage de fichier'],
    ['compare',           'compare(${1:texte}, "${2:motif*}")',                       'Correspondance avec wildcards (TRUE/FALSE)'],
    ['hexaToUnicode',     'hexaToUnicode("${1:x9}")',                                'Hex → caractère Unicode'],
    ['number',            'number(${1:texte})',                                       'Texte → numérique'],
    ['strip',             'strip(${1:texte}, "${2: }")',                              'Supprime caractères dans la chaîne'],
    ['debug',             'debug(${1:valeur}, NL)',                                   'Log debug (LOGLEVEL >= 2)'],
    ['new',               'new("${1:collection}")',                                   'Nouvel enregistrement de collection'],
    ['next',              'next(${1:qCursor})',                                       'Avance le curseur de requête'],
    ['flush',             'flush(${1:0}, ${2:80}, ${3:NL})',                          'Vide le buffer positionnel'],
    ['put',               'put(${1:1}, ${2:1}, ${3:valeur})',                         'Écrit dans le buffer à une position'],
    ['edierrorlist',      'edierrorlist(${1:SEGMENT})',                               'Liste des erreurs EDI d\'un segment'],
    ...
    ['sendGisEmissionMessage', 'sendGisEmissionMessage(${1:pDOC.UUID})',             'Émission message GIS'],
    ['sendKsefInvoiceMessage', 'sendKsefInvoiceMessage(${1:pDOC.UUID})',             'Soumission facture KSeF (Pologne)'],
    ['db.integrity.find', 'db.integrity.find(${1:tQuery})',                          'Requête base d\'intégrité Generix'],
    ['db.xxx.find',       'db.${1:collection}.find("${2:filtre JSON}")',             'Requête MongoDB → curseur'],
    ['db.xxx.count',      'db.${1:collection}.count("${2:filtre JSON}")',            'Compte enregistrements MongoDB'],
    ['db.xxx.aggregate',  'db.${1:collection}.aggregate("${2:pipeline}")',           'Agrégation MongoDB → curseur'],
  ];
